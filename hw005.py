# 5주 차 과제: [운영체제] 페이징(Paging) 주소 변환기
# 개념 연결: 가상 메모리의 핵심인 페이징 시스템에서, 가상 주소(논리 주소)가 페이지 테이블을 거쳐 어떻게 물리 주소(실제 메모리 주소)로 변환되는지 구현합니다.
# 과제 안내문: "페이지 크기가 4KB(4096 바이트)일 때, 가상 주소를 물리 주소로 변환하는 간단한 함수를 완성하세요.
